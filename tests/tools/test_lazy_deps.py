...
class TestIsSatisfiedVersionAware:
    ...
    def test_exact_pin_match_returns_true(self, monkeypatch):
        self._fake_version(monkeypatch, {"honcho-ai": "2.1.1"})
        assert ld._is_satisfied("honcho-ai==2.1.1") is True

    def test_exact_pin_mismatch_returns_false(self, monkeypatch):
        # Installed 2.1.0, spec requires 2.1.1 → False (needs upgrade).
        self._fake_version(monkeypatch, {"honcho-ai": "2.1.0"})
        assert ld._is_satisfied("honcho-ai==2.1.1") is False
